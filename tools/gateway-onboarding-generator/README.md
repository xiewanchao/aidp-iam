...
| 应用名 | 请求超时时间/s | 限流请求数/s/m/h/d | TCP 最大连接数 | 重试 |
|---|---:|---|---:|---|
| KnowledgeBase | 30s | 60/Minute | 1000 | 2 次，5xx/gateway-error/connect-failure，单次 2s |
| 业务应用名 | 例如 30s | 例如 100/Second | 例如 2000 | 次数、触发条件、单次超时、退避间隔 |
...
勾选“Gateway 级 TCP 连接数限制”后，会生成 `ClientTrafficPolicy.spec.connection.connectionLimit`。默认限制整个 Gateway，也可以选择只作用于 `http` 或 `https` listener。
