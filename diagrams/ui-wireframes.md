...
**核心原则**：`path_rules` 表只有 `(path_prefix, required_group)` 两个核心字段，但管理员的心智有两种：

- **视角 A（路径维度）**："这条路径要给谁开？"——从【路径规则】页进入
- **视角 B（组维度）**："这个组能访问什么？"——从【用户组 → 组详情】进入

两个页面**操作的是同一张表**，可以从任一侧切入。两侧都用"**列表选取（穿梭框）**"模式，路径按应用分类，组从组列表选择。
...
│  │    ◯ /knowledgebase/v1/kb              未配置           │   │
│  │    ◯ /knowledgebase/v1/admin/*         已配 → kb-admins │   │
│  │    ● /knowledgebase/v1/upload          未配置  ← 已选   │   │
│  │    ◯ /knowledgebase/v1/share                            │   │
│  │  ▾ memory  (3 paths)                                    │   │
│  │    ◯ /memory/v1/get                                     │   │
│  │    ◯ /memory/v1/admin/*                已配 → admins    │   │
│  │  ▾ llm  (4 paths)                                       │   │
│  │    ◯ /llm/v1/chat                                       │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
...
│  │  ◯ memory-admins       应用预置  · 5 成员                │   │
│  │  ◯ dev-team            自定义    · 30 成员               │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  说明: [限制 KB 上传仅 kb-creators                ]              │
│                                                                 │
│                          [取消]    [✓ 创建规则]                  │
└─────────────────────────────────────────────────────────────────┘
```
...
- 已配置的路径显示当前 group（避免误操作）
- 顶部支持**搜索 + 应用筛选**
- 组选择是单选（path_rules 表 path_prefix 唯一，每条 path 只能映射一个 group）
...
- 左侧路径**按应用分组折叠**，可批量勾选
- 已被其他组配置的路径有 ⚠ 提示，**避免误"抢占"**
- 顶部**搜索 + 应用筛选**双重过滤
- 保存后批量写入 path_rules 表（每个 path 一条记录）

#### 双向跳转

- 路径规则页 → 点击 group 名 → 跳到该组详情的"可访问路径" Tab
- 组详情 → 点击某条路径 → 跳到路径规则编辑弹窗

#### 一致性保证

- 两个入口写的是同一张 `path_rules` 表
- 操作完成后通过同一个 OPAL 通道下发到 OPA
- 任意一侧操作，另一侧立即可见
