...
REPO_ROOT=$(realpath "${CURRENT_PATH}/../../..")
GATEWAY_MANAGER_SRC="${REPO_ROOT}/apps/gateway-manager"
GATEWAY_MANAGER_BUILD="${REPO_ROOT}/build/docker/gateway-manager"
GATEWAY_CHART_DIR="${REPO_ROOT}/deploy/helm/aidp-gateway"
...
IMAGE_OUTPUT_DIR="${IMAGE_OUTPUT_DIR:-${REPO_ROOT}/artifacts/images/gateway/${ARCH}}"
CHART_OUTPUT_DIR="${CHART_OUTPUT_DIR:-${REPO_ROOT}/artifacts/charts}"
...
function prepare_gateway_manager_context() {
    ...
    cp "${GATEWAY_MANAGER_BUILD}/requirements.txt" "${context_dir}/requirements.txt"
    cp -R "${GATEWAY_MANAGER_SRC}/app" "${context_dir}/app"
}
...
function package_gateway_chart() {
    helm package "${GATEWAY_CHART_DIR}" -d "${CHART_OUTPUT_DIR}"
}
