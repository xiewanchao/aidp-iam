...
#   - tokens for 2 users: admin (T_ADMIN) and normal-user (T_USER)
#     The aidp realm only has these two users (created by init-keycloak.py).
#     Tests use T_ADMIN as alice (resource owner) and T_USER as bob.
#   - assert_eq / assert_in / assert_match / assert_contains / assert_not_contains
#   - http_code / http_body / json_get
#   - NH / NHC / NPOST / NPOSTC / NPUT / NPUTC / NDEL
#   - acl helpers (acl_count / acl_owner / acl_max_id / acl_latest_resource_id)
#   - section / banner / print_summary
# ============================================================================
set -uo pipefail
...
acl_max_id() {
  ...
  kubectl -n "$KEYCLOAK_NS" exec postgres-0 -c postgres -- \
    psql -U keycloak -d iam -tA -c \
    "SELECT COALESCE(MAX(id), 0) FROM resource_acl WHERE object_path LIKE '$app/%'" 2>/dev/null \
    | tr -d ' \r\n'
}
...
acl_latest_resource_id() {
  ...
  kubectl -n "$KEYCLOAK_NS" exec postgres-0 -c postgres -- \
    psql -U keycloak -d iam -tA -c \
    "SELECT split_part(object_path, '/', -1) FROM resource_acl WHERE object_path LIKE '$app/%' AND id > $prev_max ORDER BY id DESC LIMIT 1" 2>/dev/null \
    | tr -d ' \r\n'
}
