#!/usr/bin/env bash
# ============================================================================
# walkthrough.sh — MemoryBank API interactive narration (no assertions)
#
...
echo "GET /MemoryBank/health"
http_body "$GATEWAY/MemoryBank/health" | python -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2))" 2>/dev/null || true

echo ""
section "1. alice creates an Instance"
echo "PUT /MemoryBank/Tenants/$TID/Instances/$INST"
...
sleep 2
echo "  → ACL owner: $(acl_owner MemoryBank "$INST")"

echo ""
section "2. alice creates a Memory"
echo "PUT /MemoryBank/Tenants/$TID/Instances/$INST/Memories/mem-wt"
...
echo "POST /MemoryBank/Tenants/$TID/Instances/$INST/Memories/Query"
NPOST "$T_ALICE" "$BASE/Instances/$INST/Memories/Query" '{"query":"red"}' \
  | python -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2))" 2>/dev/null || true

echo ""
section "4. alice creates a Template"
echo "PUT /MemoryBank/Tenants/$TID/Instances/$INST/Templates/tpl-wt"
...
echo "POST /MemoryBank/Tenants/$TID/Instances/$INST/Templates/tpl-wt/Filters"
...
echo "POST /MemoryBank/Tenants/$TID/Instances/$INST/Templates/tpl-wt/LLMExtraction"
...
echo "GET /MemoryBank/Tenants/$TID/Instances/$INST  (as bob)"
...
SHARE=$(NPOST "$T_ALICE" "$GATEWAY/acl/v1/resources/$INST/permissions" \
  "{\"subject_type\":\"user\",\"subject_id\":\"$BOB_UID_WT\",\"permission\":\"viewer\",\"app_name\":\"MemoryBank\",\"resource_type\":\"Instances\"}")
...
echo "DELETE /MemoryBank/Tenants/$TID/Instances/$INST"
echo "  → HTTP $(NDEL "$T_ALICE" "$BASE/Instances/$INST") (expect 204)"
sleep 2
echo "  → Instance ACL count: $(acl_count MemoryBank "$INST") (expect 0)"
