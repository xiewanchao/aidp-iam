...
1. [aidp-gateway](../../aidp-gateway/) → 提供 Gateway `eg`、CRD、控制器
2. [aidp-iam](../../aidp-iam/) → 提供 pep-proxy（鉴权）、resource-sync（ACL 同步）
...
```
aidp-gateway      网关基建（必装）
     ↓
aidp-iam          IAM 平台 + 内置 KB/Rubik/Memory 注册数据（必装）
     ↓
