...
| ACL 授权更新 | tenant、调用者身份、object_path | 管理服务 | 越权授权 | 非 owner 或非管理员授权成功 | 普通 Viewer/无权限用户可给第三方授予资源权限 | 是 | owner 校验缺失、组权限展开错误、管理员判断过宽、token 注入头可信边界错误 | API 内部校验 owner/管理员；跨租户对象拒绝；pep-proxy 注入可信身份 | 未授权用户可扩大访问范围，造成权限泄露 | 极高 | 1.立即撤销异常 ACL 2.审计授权记录和调用者 3.修复 owner/admin 判断 4.恢复时间 5min | 使用 Viewer 用户调用 PUT ACL 给第三方授权 |
| ACL 授权更新 | tenant、object_path | 安全服务 | 跨租户写入 | 跨租户 ACL 被接受 | t1 用户或管理员写入 t2 对象路径 ACL | 是 | object_path tenant 解析错误、路径规范绕过、大小写或编码未标准化 | 写入时校验对象路径 tenant；跨租户返回 `400/403` | 可能造成跨租户资源可见或可操作 | 极高 | 1.删除跨租户 ACL 2.修复路径解析和规范化 3.补充跨租户回归测试 4.恢复时间 5min | 用 t1 token 写入 `/KnowledgeBase/Tenants/t2/...` ACL |
...
| ACL 查询 | object_path、分页参数 | 管理服务 | 查询超时 | ACL 查询响应超时 | 单对象或单用户授权记录较多时页面查询超时 | 是 | 索引缺失、分页缺失、前缀查询扫描过大、数据库连接池耗尽 | `resource_acl` 按 tenant/object/user 建索引；列表查询分页 | 权限页面不可用，管理员无法排障或撤销权限 | 中 | 1.增加或修复索引 2.降低单次查询范围 3.重启连接池或扩容 DB 4.恢复时间 10min | 构造 10 万条 ACL 后查询单用户授权 |
...
| 资源创建自动 Owner | 创建请求、响应体资源 ID | 同步服务 | 错误归属 | Owner 写给错误主体或错误对象 | 创建者、tenant 或 object_path 解析错误导致 ACL 归属错误 | 是 | `x-auth-*` header 缺失或被伪造、路径解析错误、响应 ID 拼接错误、并发请求上下文串扰 | Gateway 清理客户端内部头；resource-sync 使用鉴权后注入身份；object_path 按统一 URL 构造 | 创建者无权限，其他用户意外获得 Owner | 极高 | 1.删除错误 ACL 2.按请求 ID 对齐日志和 DB 记录 3.修复身份/路径构造逻辑 4.恢复时间 10min | 客户端注入伪造 `X-Auth-User-Id` 或并发创建多个资源 |
| pending_acl 补偿 | pending_acl 任务 | 同步服务 | 重试堆积 | 失败同步长期未补偿 | ACL 写入/删除失败任务持续堆积，最终一致性无法达成 | 是 | retry worker 未启动、next_retry 计算错误、DB 持续不可用、最大重试次数过小 | `pending_acl` 记录 retry_count、last_error、next_retry；后台周期重试 | 创建者权限缺失或删除残留长期存在 | 高 | 1.查看 pending 任务数量和错误 2.恢复 DB 或 worker 3.手工触发重试 4.恢复时间 10min | 停止 DB 后批量创建资源，再恢复 DB 观察 pending 清理 |
...
| 集合查询过滤 | collection path、page_size | 同步服务 | 结果过大 | `X-Allowed-Ids` 过长或被截断 | 用户可见资源过多导致 header 超长，后端无法正确过滤 | 是 | 未限制单页大小、资源授权过多、header 大小超过 Envoy 或后端限制、未使用分页 | 列表查询分页，单页上限 500；注入 `X-Allowed-Total` | 列表接口返回异常、漏数据或后端 431/500 | 高 | 1.降低 page_size 2.引入分页游标或 ListAllowedIds 接口 3.调整 header 限制 4.恢复时间 5min | 给用户授权超过 500 个资源后请求集合接口 |
| 内部 Header 防伪 | 客户端 headers | 安全服务 | Header 伪造 | 客户端伪造 `X-Allowed-Ids` 或 `X-Auth-*` 生效 | 未授权用户通过伪造内部 header 影响后端过滤或身份 | 是 | Gateway 未清理客户端内部头、后端直接信任外部 header、大小写变体未覆盖 | 设计要求 Gateway 清理客户端 `X-Auth-*`、`X-Allowed-Ids`；身份由 pep-proxy 注入 | 列表资源泄露或身份越权 | 极高 | 1.阻断受影响路由 2.修复 header 清理规则 3.审计访问日志和 ACL 4.恢复时间 5min | 客户端直接发送 `X-Allowed-Ids=*` 或伪造 `X-Auth-User-Id` |
...
| 资源级鉴权 | request_path、resource_patterns | 鉴权服务 | 保护范围漏配 | 需要资源级鉴权的路径未进入 ACL 检查 | 资源实例路径被当成普通路径放行，只做路径级鉴权 | 是 | Manifest 未声明 resource_patterns、路径模板不匹配、HTTP method 映射缺失、应用 namespace 错误 | Manifest 注册同步 resource_patterns；路径级 OPA 先执行 | 有路径权限的用户可访问未授权资源实例 | 极高 | 1.补充 resource_patterns 并重新注册 Manifest 2.验证资源实例访问 3.等待 OPA/DB 刷新 4.恢复时间 5min | 删除某资源类型的 resource_patterns 后访问单资源 |
| 角色矩阵判定 | role_path、HTTP method | 鉴权服务 | 权限矩阵错误 | Viewer/Contributor/Owner 操作边界错误 | 角色对应的 GET/PUT/DELETE 权限与设计不一致 | 是 | 角色路径配置错误、method 归类错误、自定义角色回调失败、继承角色覆盖错误 | 角色矩阵集中在 pep-proxy 判定；QueryACLs 返回 role_path 便于核对 | 用户被误拒或获得超过角色的写/删权限 | 高 | 1.核对角色矩阵和 role_path 2.修复 method 映射 3.补充 Viewer/Contributor/Owner 回归 4.恢复时间 5min | 用 Viewer 调 PUT/DELETE，用 Contributor 调 DELETE |
...
| ACL 级联删除 | delete_prefix、object_path | 同步服务 | 过度删除 | 删除父路径时误删无关 ACL | 前缀匹配条件过宽，删除了相邻资源或其他租户 ACL | 是 | LIKE 前缀未加路径分隔符、tenant 条件缺失、object_path 未规范化 | 删除条件为 `object_path = prefix OR object_path LIKE prefix/%`；带 tenant 维度 | 无关资源权限丢失，造成大面积误拒 | 极高 | 1.从备份恢复误删 ACL 2.修复 delete_prefix 条件 3.重新跑级联删除测试 4.恢复时间 30min | 删除 `/KnowledgeBases/kb-1` 时验证 `/KnowledgeBases/kb-10` ACL 是否保留 |
| 组级对象权限 | group_name、permissions[] | 管理服务 | 批量部分失败 | 组级 ObjectPermissions 部分写入 | 批量配置返回成功但部分对象权限未生效 | 是 | 事务边界不完整、个别 object_path 非法、组不存在、DB 中途失败 | 事务批量写入/删除；返回 upserted/deleted 数量 | 组内用户权限不一致，页面显示与实际访问不符 | 高 | 1.核对返回数量和 QueryACLs 结果 2.修复非法项后重试整批 3.必要时回滚本批 4.恢复时间 5min | 批量请求中混入跨租户 object_path 和合法 object_path |
| 并发授权撤销 | 同一 user_path/object_path | 管理服务 | 竞态覆盖 | 并发修改导致最终角色错误 | 多个管理员同时授权/撤销同一主体同一对象，最终状态不可预期 | 是 | 并发 upsert/delete 缺少事务隔离、页面重复提交、旧请求后到覆盖新请求 | 唯一键约束 `(tenant_id,user_path,object_path)`；upsert 幂等 | 用户权限与最后一次有效操作不一致 | 中 | 1.查询审计和最终 ACL 2.重新写入目标角色 3.增加乐观锁或更新时间校验 4.恢复时间 3min | 并发执行 Owner 授权和 DELETE ACL |
