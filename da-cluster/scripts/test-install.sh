...
#   IT-AT-006  core da-cluster/scripts/test.sh passes
...
#
# Examples:
#   bash da-cluster/scripts/test-install.sh
#   bash da-cluster/scripts/test-install.sh --mock memory
#   bash da-cluster/scripts/test-install.sh --mock kb --skip-mock-build
#   bash da-cluster/scripts/test-install.sh --no-kind --skip-reinstall
#   bash da-cluster/scripts/test-install.sh --include-failure-demo
# ============================================================================
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DA_CLUSTER_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_DIR="$(cd "$DA_CLUSTER_DIR/.." && pwd)"
...
if [ -z "$RESULT_DIR" ]; then
  RESULT_DIR="$DA_CLUSTER_DIR/test-results/install-$(date +%Y%m%d-%H%M%S)"
fi
mkdir -p "$RESULT_DIR"
...
ensure_kind_cluster_for_runtime_images() {
  ...
  local kind_config="$DA_CLUSTER_DIR/kind-config.yaml"
...
mock_config() {
  ...
  case "$selected" in
    kb)
      ...
      MOCK_CONTEXT="$REPO_DIR/mock-kb"
      MOCK_CHART="$REPO_DIR/mocks/package-mock-kb/charts/aidp-mock-kb"
      MOCK_TEST="$REPO_DIR/mocks/package-mock-kb/test/test.sh"
    ...
    memory)
      ...
      MOCK_CONTEXT="$DA_CLUSTER_DIR/images/mock-memory"
      MOCK_CHART="$REPO_DIR/mocks/package-mock-memory/charts/aidp-mock-memory"
      MOCK_TEST="$REPO_DIR/mocks/package-mock-memory/test/test.sh"
    ...
    dataagent)
      ...
      MOCK_CONTEXT="$DA_CLUSTER_DIR/images/mock-dataagent"
      MOCK_CHART="$REPO_DIR/mocks/package-mock-dataagent/charts/aidp-mock-dataagent"
      MOCK_TEST="$REPO_DIR/mocks/package-mock-dataagent/test/test.sh"
...
case_start "IT-AT-006: core IAM test suite" \
  "Run da-cluster/scripts/test.sh and require exit code 0."
...
if [ "$RUN_REINSTALL" = true ]; then
  ...
  case_start "IT-AT-011: second core test after skip-build reinstall" \
    "Run da-cluster/scripts/test.sh again after the skip-build reinstall."
